...
WHERE
{
  ...
  # a predicate containing the word "affiliatedWith" in its label
  ?Person ?affiliatedWith ?Institute .
  ?affiliatedWith rdfs:label ?predicate1 .
  FILTER(CONTAINS(?predicate1, "affiliatedWith")) .
  
  # Get Person and PlasmaSource instances that are connected with
  # a predicate containing the word "experiencedIn" in its label
  ?Person ?experiencedIn ?PlasmaSource .
  ?experiencedIn rdfs:label ?predicate2 .
  FILTER(CONTAINS(?predicate2, "experienced")) .
